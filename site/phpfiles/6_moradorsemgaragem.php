...
$target = 'anexos/Morador_sem_Garagem/'.$newname;
...
if($_POST['formSubmit'] == "Submeter Candidatura"){
	...
	$sql = "Update requerente set tipoRequerente='Morador sem garagem' where idNumFiscal=".$id;
	...
	while($row = $result->fetch_assoc()) {
		$pdf->Cell(0,10,'Formulario para Morador Sem Garagem',0,0,'C');$pdf->Ln();$pdf->Ln();
	...
	$pdf->Output('C:\xampp\htdocs\formulario\phpfiles\anexos\Morador_sem_Garagem/FormularioMoradorSemGaragem.pdf','F');
	
	
	$date = date('Ymd');
	$hora = date('His');
	
	$zipFile = "./anexos/Morador_sem_Garagem/".$id."anexos".$date.$hora.".zip";
	$zipArchive = new ZipArchive();

	if (!$zipArchive->open($zipFile, ZIPARCHIVE::CREATE))
		die("Failed to create archive\n");

	$zipArchive->addGlob("./anexos/Morador_sem_Garagem/*.pdf");
	$zipArchive->addGlob("./anexos/Morador_sem_Garagem/*.png");
	$zipArchive->addGlob("./anexos/Morador_sem_Garagem/*.jpeg");
	$zipArchive->addGlob("./anexos/Morador_sem_Garagem/*.jpg");
	if (!$zipArchive->status == ZIPARCHIVE::ER_OK)
		echo "falha ao criar o zip\n";

	$zipArchive->close();
	
	unlink('./anexos/Morador_sem_Garagem/RegistoVeiculo.'.$ext1);
	unlink('./anexos/Morador_sem_Garagem/comprovativoResidencia.'.$ext2);	
	unlink('./anexos/Morador_sem_Garagem/CartaConducao.'.$ext3);
	unlink('./anexos/Morador_sem_Garagem/Procuracao.'.$ext4);
	unlink('./anexos/Morador_sem_Garagem/FormularioMoradorSemGaragem.pdf');
...
function sendMail($id,$date,$hora){
	...
	$email->FromName  = '';
	$email->Subject   = 'Novo Formulario para Morador Sem Garagem Nr Fiscal '.$id;
	$email->Body      = 'Novo pedido de acesso a zona pedonal para um morador sem garagem';
	$email->AddAddress('a67740@alunos.uminho.pt');

	$email->AddAttachment("./anexos/Morador_sem_Garagem/".$id."anexos".$date.$hora.".zip");
