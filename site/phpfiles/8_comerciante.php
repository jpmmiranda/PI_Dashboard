...
$newname = "ComprovativoEstabelecimento.".$ext2; 
$target = 'anexos/Comerciante/'.$newname;
move_uploaded_file( $_FILES['comprovativoResidencia']['tmp_name'], $target);

$info = pathinfo($_FILES['comerciante']['name']);
$ext3 = $info['extension']; // get the extension of the file
$newname = "DeclaracaoEstabelecimento.".$ext3; 
$target = 'anexos/Comerciante/'.$newname;
move_uploaded_file( $_FILES['comerciante']['tmp_name'], $target);
...
if($_POST['formSubmit'] == "Submeter Candidatura"){
	...
	$sql = "Update requerente set tipoRequerente='Comerciante' where idNumFiscal=".$id;
	...
	while($row = $result->fetch_assoc()) {
		$pdf->Cell(0,10,urldecode(em('Formulário de Comerciante')),0,0,'C');$pdf->Ln();$pdf->Ln();$pdf->Ln();
	...
	$pdf->Output('C:\xampp\htdocs\formulario\phpfiles\anexos\Comerciante\FormularioComerciante.pdf','F');
	
	$date = date('Ymd');
	$hora = date('His');
	
	$zipFile = "./anexos/Comerciante/".$id."anexos".$date.$hora.".zip";
	$zipArchive = new ZipArchive();

	if (!$zipArchive->open($zipFile, ZIPARCHIVE::CREATE))
		die("Failed to create archive\n");

	$zipArchive->addGlob("./anexos/Comerciante/*.pdf");
	$zipArchive->addGlob("./anexos/Comerciante/*.png");
	$zipArchive->addGlob("./anexos/Comerciante/*.jpeg");
	$zipArchive->addGlob("./anexos/Comerciante/*.jpg");
	if (!$zipArchive->status == ZIPARCHIVE::ER_OK)
		echo "falha ao criar o zip\n";

	$zipArchive->close();
	
	unlink('./anexos/Comerciante/RegistoVeiculo.'.$ext1);
	unlink('./anexos/Comerciante/ComprovativoEstabelecimento.'.$ext2);
	unlink('./anexos/Comerciante/DeclaracaoEstabelecimento.'.$ext3);
	unlink('./anexos/Comerciante/FormularioComerciante.pdf');
...
function sendMail($id,$date,$hora){
	...
	$email->FromName  = '';
	$email->Subject   = 'Novo Formulario de Comerciante Nr Fiscal '.$id;
	$email->Body      = 'Novo pedido de acesso a zona pedonal do centro de braga';
	$email->AddAddress('a67740@alunos.uminho.pt');
	
	$email->AddAttachment("./anexos/Comerciante/".$id."anexos".$date.$hora.".zip");
